...
@Directive({
  selector: '[appEditOrderIntersectionAlt]'
})
export class EditOrderIntersectionAltDirective implements OnInit, AfterViewInit, OnDestroy{
  ...
  checkVisible(): void {
    this.editOrderObserver = new IntersectionObserver(entry => {
      entry[0].intersectionRatio === 0 ? $('#toRight').css({ translate: '0 0' })
      : $('#toRight').css({ translate: '100% 0' });
    });
  };
}
